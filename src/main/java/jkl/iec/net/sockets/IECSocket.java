...
//	      }
	      ...
	      if (iecSocketParameter.iecSocketType == IECSocketType.IECClient) {
	         if (getIecstatus() == IECSocketStatus.INIT) {
	        	 send_Stop();	        	 
	          }
    	      if (iecSocketParameter.t3 < new Date().getTime()) {
			      log.finer("(t3) exp. send polling");
...
//    int id;
    ...
    private void setRemoteAddress(String i) {
    	remoteAddress.set(i);
    }
    
    private SimpleIntegerProperty idx = new SimpleIntegerProperty();
    public SimpleIntegerProperty idxProperty() {return idx;}
...
//	private IECSocketStatus iecstatus= IECSocketStatus.INIT;
	private SimpleObjectProperty<IECSocketStatus> iecstatus= new SimpleObjectProperty<IECSocketStatus>(IECSocketStatus.INIT);
	private SimpleObjectProperty<IECSocketStatus> iecstatusProperty() {return iecstatus;}
	    ...
	    if (iecSocketParameter.iecSocketType == IECSocketType.IECClient) {
  		  ...
  		  if (s!= iecstatus.get()) {
        		  log.finest(s.toString());
	       		  if (s==IECSocketStatus.StartDT) {
	    	    	  send_Start();
	    		  }
	    		  if (s==IECSocketStatus.StopDT) {
	    	    	  send_Stop();
	    		  }
  		  	}
	    }
...
//		System.out.println("LOGGER name  "+log.getName());
	...
	public IECSocket(Socket sock,IIECnetActionListener a) {
	 ...
	 public void setSocket(Socket s) {
		 this.socket = s;
		 setRemoteAddress(s.getRemoteSocketAddress().toString());
	 }
...
//				trace(TraceLevel.DEBUG,"Socket Received : "+IPcount+ "bytes");
		    	if (IPcount !=-1) {
		    		...
		    		actionevent = new ActionEvent(this,ActionEvent.ACTION_PERFORMED,IECSocketStatus.RemoteClose.toString());
...
//			    		acttionlistener.actionPerformed(actionevent);
	...
	private void close() {
 		if (!socket.isClosed()) {
 			...
 			try {
 				socket.close();
				actionevent = new ActionEvent(this,ActionEvent.ACTION_PERFORMED,IECSocketStatus.Close.toString());
	    		ThreadAction a= new ThreadAction(actionevent);
	    		a.start();
	    		//acttionlistener.actionPerformed(actionevent);
 			} catch (IOException e) {
	 ...
	 private void doAPCI(APCIFrame apci) {
		 if ((apci == APCIFrame.APCI_U_start)||(apci == APCIFrame.APCI_U_start_ack)) {
			...
			setIecstatus(IECSocketStatus.StartDT);
	    	actionevent = new ActionEvent(this,ActionEvent.ACTION_PERFORMED,IECSocketStatus.StartDT.toString());
    		ThreadAction a= new ThreadAction(actionevent);
    		a.start();
		 ...
		 if ((apci == APCIFrame.APCI_U_stop)||(apci == APCIFrame.APCI_U_stop_ack)) {
			...
			setIecstatus(IECSocketStatus.StopDT);
			actionevent = new ActionEvent(this,ActionEvent.ACTION_PERFORMED,IECSocketStatus.StopDT.toString());
    		ThreadAction a= new ThreadAction(actionevent);
    		a.start();
    		//acttionlistener.actionPerformed(e1);
		 	}
	...
	// Determine what type of 104 Frame (APCI) is received  and notify App. if I_Frame received;
		 ...
		 APCIFrame apci = getAPCI();    // what frame i have received ??
			if (apci== APCIFrame.APCI_I_Frame) {
				txt=String.format("RX: %s [%05d] %s","_i_",vr,IECFunctions.byteArrayToHexString(APDU,0,APDUlength));
				log.fine(txt);	
				}
			if (apci== APCIFrame.APCI_S_Frame)  {
				txt= String.format("RX: %s [%05d] %s","_s_",vs,IECFunctions.byteArrayToHexString(APDU,0,APDUlength));
				log.fine(txt);
			}
			...
			if ((apci!= APCIFrame.APCI_I_Frame)&&(apci!= APCIFrame.APCI_S_Frame))  {
				...
				log.fine("RX: "+txt+IECFunctions.byteArrayToHexString(APDU,0,APDUlength));
			}
			if (apci== APCIFrame.APCI_I_Frame) {
				int ASDULength=0;
				...
				log.info("RX: "+IECFunctions.byteArrayToHexString(APDU,0,APDUlength));
...
//		 txt="Update_k("+iecSocketParameter.k+")   vs("+vs+")";
	 ...
	 private void send(byte[] b, int len) {
	    	...
	    	if (len>6) {
	    		log.info(tx+IECFunctions.byteArrayToHexString(b,0,len));
	    	}
	 }
