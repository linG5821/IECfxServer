...
public class IECIEEEstageController extends IECstageController {
	...
	void saveStage() {
		try {
			iob.setVal(Double.parseDouble(valueText.getText()));
		} catch (NumberFormatException e) {
			e.printStackTrace();
		}
		closeStage();
...
//				System.out.println("BOOL"+String.valueOf(arg2));
		...
		valueSlider.setValue(iob.getVal());
		valueText.setText(String.valueOf(iob.getVal()));
		maxText.setText(String.valueOf(iob.getMAX_VALUE()));
		minText.setText(String.valueOf(iob.getMIN_VALUE()));
	}
